...
## NLS logistic models before day 33 model
```{r}
...
## NLS models for A and D species
...
## Linear model fits ok with abbreviated timecourse but not with full colonization time-series
```{r}
...
fig5a <- ggplot(subset(clusts_plus_bleach_limited, treatment == "B26"),
  ...
  ylab("Total # cells\n per image-stack") +
  xlab("") +
  annotate("text", x = c(1), y = c(2325), label='underline("Observed (Exp. 3)")', parse = T, hjust=0, vjust=0) +
...
```
# Individual fits for individual nls models
```{r}
...
## resulting asymptotes, midpoints, and scales to input into model
```{r}
...
# Function to break initial day into new clusters for day 7 growth, apply growth rate to match observed logistic model
...
# Modeling logistic growth no transformation
```{r}
...
# Data Analysis for logistic growth with no growth rate transformation
...
# Square root transformation of population distribution across clusters
...
## 0.015 rate sqrt transformation
```{r, include=FALSE, results= FALSE}
...
## 0.005 rate sqrt transformation
```{r,include=FALSE, results= FALSE}
...
# Square root transformation dataset 
## Start here
```{r}
setwd("./summarycsv/migration_models/sqrt/mr0.1/")
migrationmodels.list = list.files(pattern = ".csv")
...
# faceted data to accompany figure 5B
```{r}
...
# Figure 5B - simulated population data summary
```{r}
fig5b <- ggplot(subset(migrationmodel_sum_wide.log.sqrt, migrate.rate == 0.01 & end.day %in% c(4,7,9,11,14,16,18,21,25,29,31)),
       aes(x=end.day, y=(hist_sum))) +
  geom_boxplot(aes(group = end.day), fill = "peachpuff", color = "peachpuff4") +
  stat_smooth(method = "nls",
              formula = y ~ SSlogis(x, Asym, xmid, scal),
  ...
  ylab("Total # cells\n per image-stack") +
  xlab("Days post inoculation") +
    annotate("text", x = c(1), y = c(2325), label='underline("Simulated (Exp. 3)")', parse = T, hjust=0, vjust=0) +
  theme(panel.grid.major = element_line(colour = "grey")) +
  theme(panel.grid.minor = element_line(colour = "grey"))
...
# Plots for Figure 5CDEF
## Figure 5C
```{r}
newplot0.orig <- ggplot(clusts_26B_modelmerge,aes(x=end.day,y=frequency,fill=cluster_type)) + 
 
  stat_summary(fun.data=mean_se,position=position_dodge(1.75),geom="errorbar",color = "black", width=1.25) +
    stat_summary(fun=mean,position=position_dodge(width=1.75),geom="bar", color = "black", alpha = 1, width = 1.25) +
  theme_linedraw() +
scale_fill_manual(values = c("#000000", "#E69F00", "#56B4E9", "#009E73","#F0E442"), name = "Cluster size (cells)", labels = c("1","2","3 to 4","5 to 10","over 10")) + ylab("Mean # of clusters      \n per image-stack     ") + scale_x_continuous(limits = c(0,32.9), breaks = c(0,2,4,7,9,11,14,16,18,21,25,31), minor_breaks = seq(0,100,1)) + xlab("") +
  scale_color_manual(values = c("black","purple"))+ theme(legend.position="bottom") + guides(fill = guide_legend(nrow=1, byrow=TRUE)) + coord_cartesian(xlim = c(2.75,31.25),ylim =c(0,85)) + 
  scale_y_continuous(breaks=seq(-1000,1000,by=20),labels=abs(seq(-1000,1000,by=20))) +
  annotate("text", x = c(2), y = c(70), label='underline("Observed population growth")', parse = T, hjust=0, vjust=0) +
  theme(panel.grid.major = element_line(colour = "grey")) +
  theme(panel.grid.minor = element_line(colour = "grey"))
...
scale_fill_manual(values = c("#000000", "#E69F00", "#56B4E9", "#009E73","#F0E442"), name = "Cluster size (cells)", labels = c("1","2","3 to 4","5 to 10","over 10")) + ylab("Mean # of clusters      \n per image-stack     ") + scale_x_continuous(limits = c(0,32.9), breaks = c(0,2,4,7,9,11,14,16,18,21,25,31), minor_breaks = seq(0,100,1)) + xlab("") +
  scale_color_manual(values = c("black","purple"))+ theme(legend.position="bottom") + guides(fill = guide_legend(nrow=1, byrow=TRUE)) + coord_cartesian(xlim = c(2.75,31.25),ylim =c(0,85)) + 
  scale_y_continuous(breaks=seq(-1000,1000,by=20),labels=abs(seq(-1000,1000,by=20))) +
  annotate("text", x = c(2), y = c(70), label='underline("Model 1: no migration, uniform growth")', parse = T, hjust=0, vjust=0) +
  theme(panel.grid.major = element_line(colour = "grey")) +
  theme(panel.grid.minor = element_line(colour = "grey"))
...
scale_fill_manual(values = c("#000000", "#E69F00", "#56B4E9", "#009E73","#F0E442"), name = "Cluster size (cells)", labels = c("1","2","3 to 4","5 to 10","over 10")) + ylab("Mean # of clusters      \n per image-stack     ") + scale_x_continuous(limits = c(0,32.9), breaks = c(0,2,4,7,9,11,14,16,18,21,25,31), minor_breaks = seq(0,100,1)) + xlab("") +
  scale_color_manual(values = c("black","purple"))+ theme(legend.position="bottom") + guides(fill = guide_legend(nrow=1, byrow=TRUE)) + coord_cartesian(xlim = c(2.75,31.25),ylim =c(0,85)) + 
  scale_y_continuous(breaks=seq(-1000,1000,by=20),labels=abs(seq(-1000,1000,by=20))) +
  annotate("text", x = c(2), y = c(70), label='underline("Model 2: migration, uniform growth")', parse = T, hjust=0, vjust=0) +
  theme(panel.grid.major = element_line(colour = "grey")) +
  theme(panel.grid.minor = element_line(colour = "grey"))
...
## Figure 5F
```{r}
newplot0.01sqrt <- ggplot(mm_log_migrate_one.subset3.01.sqrt,aes(x=end.day,y=frequency,fill=cluster_type)) + 
 
  stat_summary(fun.data=mean_se,position=position_dodge(1.75),geom="errorbar",color = "black", width=1.25) +
    stat_summary(fun=mean,position=position_dodge(width=1.75),geom="bar", color = "black", alpha = 1, width = 1.25) +
  theme_linedraw() +
scale_fill_manual(values = c("#000000", "#E69F00", "#56B4E9", "#009E73","#F0E442"), name = "Cluster size (cells)", labels = c("1","2","3 to 4","5 to 10","over 10")) + ylab("Mean # of clusters      \n per image-stack     ") + scale_x_continuous(limits = c(0,32.9), breaks = c(0,2,4,7,9,11,14,16,18,21,25,31), minor_breaks = seq(0,100,1)) + xlab("Days post inoculation") +
  scale_color_manual(values = c("black","purple"))+ theme(legend.position="bottom") + guides(fill = guide_legend(nrow=1, byrow=TRUE)) + coord_cartesian(xlim = c(2.75,31.25),ylim =c(0,85)) + 
  scale_y_continuous(breaks=seq(-1000,1000,by=20),labels=abs(seq(-1000,1000,by=20))) +
  annotate("text", x = c(2), y = c(70), label='underline("Model 3: migration, density-dependent growth")', parse = T, hjust=0, vjust=0) +
  theme(panel.grid.major = element_line(colour = "grey")) +
  theme(panel.grid.minor = element_line(colour = "grey"))
...
plot_figure_5_part1 <- ggarrange( 
                       ...
                       legend = "none", heights = c(1,-.1,1))
plot_figure_5_part1

```
## Figure 5CDEF combined
```{r}
plot_figure_5_part2 <- ggarrange( 
                       ...
                       common.legend = TRUE, 
                       legend = "bottom",
                       heights = c(1.33,-0.1,1.33,-0.1,1.33,-0.1,1.33))
plot_figure_5_part2

ggsave("plot_figure_5_part2a.png", height = 5.5, width = 8.67)
ggsave("plot_figure_5_part2b.png", height = 7, width = 10)
ggsave("plot_figure_5_part2c.png", height = 8, width = 8)
...
ggsave("plot_figure_5_whole1.png", height = 7, width = 10)
ggsave("plot_figure_5_whole2.png", height = 8, width = 8)

```
## Figure 5 histogram plots only
```{r}
plot_figure_5_part2 <- ggarrange( 
                           newplot0.orig, 
                           NULL,
...
ggsave("revisedfig5.png", height = 8, width = 8)
